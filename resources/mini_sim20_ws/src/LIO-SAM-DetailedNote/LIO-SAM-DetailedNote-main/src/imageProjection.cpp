...
struct VelodynePointXYZIRT
{
    ...
    // 扫描线
    uint16_t ring;      
    // 时间戳，记录相对于当前帧第一个激光点的时差，第一个点time=0
    float time;         
...
public:
    ...
    */
    PointType deskewPoint(PointType *point, double relTime)
    {
        ...
        // 在当前激光帧起止时间范围内，计算某一时刻的旋转（相对于起始时刻的旋转增量）
        float rotXCur, rotYCur, rotZCur;  //保存插值后的旋转数据
        findRotation(pointTime, &rotXCur, &rotYCur, &rotZCur);

        // 在当前激光帧起止时间范围内，计算某一时刻的平移（相对于起始时刻的平移增量）
        float posXCur, posYCur, posZCur;    //保存插值后的位置数据
    ...
    */
    void projectPointCloud()
    {
        ...
        for (int i = 0; i < cloudSize; ++i)
        {
            ...
            // 获得该点在水平面的形成的夹角   // x/y的反正切是与Y轴的夹角
            float horizonAngle = atan2(thisPoint.x, thisPoint.y) * 180 / M_PI;

            // 水平扫描角度步长，例如一周扫描1800次，则两次扫描间隔角度0.2°
            static float ang_res_x = 360.0/float(Horizon_SCAN);
            // 将与Y轴夹角的度数换算到当前点的在雷达点云中的索引值
